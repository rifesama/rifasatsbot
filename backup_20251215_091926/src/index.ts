...
bot.command('start', async (ctx) => {
  ...
  const isUserAdmin = config.telegram.adminIds.includes(ctx.from.id);
...
bot.command('lottery', async (ctx) => {
  ...
  const message = `🎰 *${lottery.name}*\n\n` +
    `💰 Precio por número: ${formatSats(lottery.ticketPrice)} sats\n` +
...
bot.action('admin_stats', isAdmin, async (ctx) => {
  ...
  // Send detailed purchases in a separate message
  if (purchases.length > 0) {
...
bot.action('admin_winner', isAdmin, async (ctx) => {
  ...
  if (!winningNumber && winningNumber !== 0) {
  ...
  if (ctx.session?.awaitingLotteryData) {
    ...
    if (data.step === 'name') {
      ...
      try {
        ...
        await ctx.reply(
          `✅ *Lotería creada exitosamente!*\n\n` +
          `📝 Nombre: ${lottery.name}\n` +
          `💰 Precio: ${formatSats(lottery.ticketPrice)} sats\n` +
        ...
        ctx.session = {};
        await ctx.reply('❌ Error creando la lotería. Verifica los datos e intenta nuevamente.');
      }
    }
    return;
  }
...
async function checkPaymentStatus(
  ...
  const maxChecks = 60; // 15 minutes (15 seconds intervals)
  let checks = 0;
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      if (isPaid) {
        ...
        // Notify user
        const drawDate = new Date(lottery.draw_date).toLocaleDateString('es-CO');
        ...
        await bot.telegram.sendMessage(
          ...
          `Recuerde que la lotería juega el ${drawDate} a las ${lottery.draw_time}.\n\n` +
