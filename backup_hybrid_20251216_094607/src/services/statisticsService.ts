...
export class StatisticsService {
  async getLotteryStats(lotteryId: number): Promise<LotteryStats> {
    ...
    const availableTickets = tickets.filter((t: any) => t.status === 'available');
    ...
    return {
      ...
      soldNumbers: soldTickets.map((t: any) => t.number),
      availableNumbers: availableTickets.map((t: any) => t.number),
    };
  }
}