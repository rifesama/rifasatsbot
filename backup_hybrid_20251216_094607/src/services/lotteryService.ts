// ============================================
// src/services/lotteryService.ts - VERSIÓN COMPLETA CON DESCRIPCIÓN
// ============================================
...
export class LotteryService {
  ...
  async createLottery(
    name: string,
    description: string | undefined,  // ← Parámetro de descripción agregado
    ticketPrice: number,
    drawDate: Date,
    drawTime: string,
    createdBy: number
    ...
    try {
      logger.info('Starting lottery creation', { 
        name,
        description,  // ← Logging de descripción
        ticketPrice, 
      ...
      logger.info('Inserting new lottery', { name, description, ticketPrice, drawDate, drawTime, createdBy });
      
      // ← INSERT actualizado con campo description
      const lotteryResult = await client.query(
        `INSERT INTO lotteries (name, description, ticket_price, draw_date, draw_time, status, created_by)
         VALUES ($1, $2, $3, $4, $5, 'active', $6) RETURNING *`,
        [name, description, ticketPrice, drawDate, drawTime, createdBy]
      );
      ...
      logger.error('Error creating lottery - ROLLBACK executed', { 
        ...
        detail: error.detail,
        name,
        description,  // ← Logging de descripción en error
        ticketPrice,
        drawDate,
        drawTime,
        createdBy
