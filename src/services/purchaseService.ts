...
import { DBMapper, PurchaseDB } from '../utils/mappers';
...
export class PurchaseService {
  async createPurchase(
    ...
    const purchase = DBMapper.purchaseFromDB(result.rows[0] as PurchaseDB);
    logger.info('Purchase created', { purchaseId: purchase.id, ticketNumber });
    return purchase;
  }
  ...
  async getPurchasesByUser(telegramUserId: number): Promise<Purchase[]> {
    ...
    return result.rows.map((row: any) => DBMapper.purchaseFromDB(row as PurchaseDB));
  }
